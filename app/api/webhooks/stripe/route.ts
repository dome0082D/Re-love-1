...
export async function POST(req: Request) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (announcementId && buyerId && checkoutType !== 'chat_unlock' && checkoutType !== 'sponsorship') {
       ...
       // delegato, salviamo anche mandate_id/curator_id e le percentuali
       // concordate al momento dell'acquisto ("fotografate" qui, così se il
       // mandato cambia più avanti, questo ordine già in corso non viene
       // toccato). Per un acquisto normale questi campi restano null,
