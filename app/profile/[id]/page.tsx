      ...
      <div className="bg-white border-b border-stone-200 pt-20 pb-10 px-6">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="w-32 h-32 bg-stone-900 rounded-[3rem] overflow-hidden flex items-center justify-center text-white shadow-xl shadow-stone-200">
            {profile.avatar_url ? (
              <img loading="lazy" decoding="async" src={fotoQuadrata(profile.avatar_url, 120).src} srcSet={fotoQuadrata(profile.avatar_url, 120).srcSet} className="w-full h-full object-cover" alt="avatar" />
            ) : (
            ...
            {announcements.map(ann => (
              <Link href={`/announcement/${ann.id}`} key={ann.id} className="group bg-white rounded-3xl overflow-hidden border border-stone-100 hover:shadow-xl transition-all">
                <div className="h-40 bg-stone-100">
                  <img loading="lazy" decoding="async" src={fotoQuadrata(ann.image_url, 400).src} srcSet={fotoQuadrata(ann.image_url, 400).srcSet} className="w-full h-full object-cover group-hover:scale-105 transition-all duration-500" alt="img" />
                </div>
                <div className="p-5">
