...
function HomePageContent() {
  ...
  // SKELETON LOADER COMPONENT (Tinte unite senza sfumature/blur) 🦴
  const SkeletonCard = ({ isTop = false }) => (
    <div className={`bg-white rounded-[2rem] p-4 shadow-sm border border-stone-100 animate-pulse flex flex-col relative overflow-hidden ${isTop ? 'h-64' : 'h-56'}`}>
  ...
  return (
    <div className="min-h-screen bg-stone-50 font-sans text-stone-900 pb-20 relative">
      ...
      {/* BARRA DI RICERCA - Colore solido senza trasparenze */}
      <div className="w-full max-w-7xl mx-auto px-4 pt-4 flex justify-center sticky top-0 z-[100] bg-stone-50 pb-2">
        <div className="relative w-full max-w-2xl shadow-md border border-stone-200 bg-white rounded-[2rem]">
          ...
          <input 
            ...
            className="w-full py-4 pl-16 pr-20 rounded-[2rem] bg-white border-none outline-none text-base md:text-lg font-black text-stone-900 focus:ring-4 focus:ring-rose-50 transition-all placeholder:text-stone-400" 
            onChange={(e) => setMainSearch(e.target.value)} 
          />
          <button 
           onClick={handleVoiceSearch}
           className={`absolute inset-y-2 right-2 w-11 h-11 md:w-12 md:h-12 rounded-[1.5rem] flex items-center justify-center transition-all ${isListening ? 'bg-rose-600 text-white animate-pulse' : 'bg-stone-100 text-rose-500 hover:bg-stone-200 shadow-sm'}`}
          >
      ...
      {/* --- GRIGLIA CON BANNER LATERALI ADATTIVI COMPATIBILI ANDROID (w-[460px] su desktop) --- */}
      <div className="w-full px-4 md:px-6 mt-6 lg:-mt-12 relative z-20 flex flex-col lg:flex-row gap-6">
        
        {/* SIDEBAR BANNER SINISTRA (SPONSOR) - ALLARGATA A 460px SENZA SFUMATURE */}
        <aside className="hidden lg:flex flex-col gap-6 w-[460px] shrink-0 self-start sticky top-24">
          <div className="bg-white border border-stone-200 rounded-[2rem] p-5 shadow-md flex flex-col items-center text-center justify-between min-h-[580px] w-full">
            <div className="w-full h-full flex flex-col">
              ...
              <div className="w-full flex-1 bg-stone-100 rounded-2xl border border-stone-200 flex items-center justify-center mb-4 overflow-hidden relative shadow-inner min-h-[380px]">
                <img 
          ...
          <div className="bg-rose-50 border border-rose-100 rounded-[2rem] p-5 shadow-md text-center">
        ...
        <main className="flex-1 w-full">
          ...
          {/* RETTANGOLO FILTRI - Colore Solido */}
          <section className="mb-12 bg-white p-6 rounded-[2.5rem] shadow-md border border-stone-200 flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              ...
              <div className="flex flex-col gap-2">
                <button onClick={handleNearbySearch} className={`p-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all shadow-md flex items-center justify-center gap-2 ${distance > 0 ? 'bg-rose-600 text-white shadow-rose-200' : 'bg-stone-900 text-white hover:bg-rose-600'}`}>
                  <MapPin size={16} />
          ...
          {/* QUATTRO RIQUADRI CENTRALI INGRANDITI - Immagini nitide, complete al 100% senza veli scuri */}
          {!catFilter && !typeFilter && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16 max-w-5xl mx-auto w-full">
              <Link href="/add?mode=new" className="group flex flex-col items-center justify-center rounded-[2rem] border border-stone-200 overflow-hidden bg-white hover:border-stone-400 transition-all shadow-md text-center aspect-square relative w-full max-w-[240px] md:max-w-none mx-auto">
                 <div className="absolute inset-0 w-full h-full overflow-hidden">
                   <img src="/nuovo.png" className="w-full h-full object-cover transition-all duration-[0.5s]" alt="Nuovo" />
                 </div>
                 <div className="absolute bottom-0 left-0 right-0 bg-stone-900 py-2.5 z-10 w-full px-2">
                   <h3 className="text-xs md:text-sm font-black uppercase italic text-white leading-tight">Vendi Nuovo</h3>
                 </div>
              </Link>
              
              <Link href="/add?mode=used" className="group flex flex-col items-center justify-center rounded-[2rem] border border-stone-200 overflow-hidden bg-white hover:border-stone-400 transition-all shadow-md text-center aspect-square relative w-full max-w-[240px] md:max-w-none mx-auto">
                 <div className="absolute inset-0 w-full h-full overflow-hidden">
                   <img src="/usato.png" className="w-full h-full object-cover transition-all duration-[0.5s]" alt="Usato" />
                 </div>
                 <div className="absolute bottom-0 left-0 right-0 bg-stone-900 py-2.5 z-10 w-full px-2">
                   <h3 className="text-xs md:text-sm font-black uppercase italic text-white leading-tight">Vendi Usato</h3>
                 </div>
              </Link>
              
              <Link href="/add?mode=gift" className="group flex flex-col items-center justify-center rounded-[2rem] border border-stone-200 overflow-hidden bg-white hover:border-stone-400 transition-all shadow-md text-center aspect-square relative w-full max-w-[240px] md:max-w-none mx-auto">
                 <div className="absolute inset-0 w-full h-full overflow-hidden">
                   <img src="/regalo.png" className="w-full h-full object-cover transition-all duration-[0.5s]" alt="Regalo" />
                 </div>
                 <div className="absolute bottom-0 left-0 right-0 bg-stone-900 py-2.5 z-10 w-full px-2">
                   <h3 className="text-xs md:text-sm font-black uppercase italic text-white leading-tight">Regalo</h3>
                 </div>
              </Link>

              <Link href="/add?mode=barter" className="group flex flex-col items-center justify-center rounded-[2rem] border border-stone-200 overflow-hidden bg-white hover:border-stone-400 transition-all shadow-md text-center aspect-square relative w-full max-w-[240px] md:max-w-none mx-auto">
                 <div className="absolute inset-0 w-full h-full overflow-hidden">
                   <img src="/baratto.png" className="w-full h-full object-cover transition-all duration-[0.5s]" alt="Baratto" />
                 </div>
                 <div className="absolute bottom-0 left-0 right-0 bg-stone-900 py-2.5 z-10 w-full px-2">
                   <h3 className="text-xs md:text-sm font-black uppercase italic text-white leading-tight">Baratto</h3>
                 </div>
              </Link>
            </div>
          )}
          ...
          <section className="mb-20">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-5 gap-6">
                {topItems.map(item => (
                  <div key={item.id} className={`group bg-white p-4 rounded-[2rem] shadow-lg border ${item.is_sponsored ? 'border-orange-400 ring-2 ring-orange-400/20' : 'border-stone-200'} hover:border-stone-400 transition-all relative overflow-hidden`}>
                    {item.is_sponsored && (
                      <div className="absolute top-0 left-0 bg-rose-600 text-white text-[8px] font-black uppercase px-3 py-1.5 rounded-br-2xl z-40 tracking-widest shadow-md">
                        TOP ✨
                      </div>
                    )}
                    <button onClick={(e) => handleToggleFavorite(e, item.id)} className="absolute top-6 right-6 z-30 bg-white border border-stone-200 w-8 h-8 flex items-center justify-center rounded-full shadow-md hover:scale-110 transition-all">
                    ...
                    <Link href={`/announcement/${item.id}`}>
                      <div className="aspect-square rounded-2xl overflow-hidden bg-stone-100 mb-4 relative border border-stone-200">
                        <img src={item.image_url || "/nuovo.png"} className="w-full h-full object-cover transition-transform duration-[0.8s]" alt={item.title} />
                      </div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-5">
                {regularItems.slice(0, visibleCount).map(item => (
                  <div key={item.id} className={`group bg-white rounded-3xl overflow-hidden shadow-md border ${item.is_sponsored ? 'border-orange-400' : 'border-stone-200'} hover:border-stone-400 transition-all flex flex-col relative`}>
                    {item.is_sponsored && (
                      <div className="absolute top-0 left-0 bg-rose-600 text-white text-[7px] font-black uppercase px-2 py-1 rounded-br-xl z-40 tracking-widest shadow-sm">
                        TOP 🌟
                      </div>
                    )}
                    <Link href={`/announcement/${item.id}`} className="aspect-square bg-stone-100 relative block overflow-hidden">
                      <button onClick={(e) => handleToggleFavorite(e, item.id)} className="absolute top-2 right-2 z-30 bg-white border border-stone-200 w-8 h-8 flex items-center justify-center rounded-full shadow-sm hover:scale-110 transition-all">
                        <Heart size={16} className={favorites.includes(item.id) ? "fill-rose-500 text-rose-500" : "text-stone-400"} />
                      </button>
                      <img src={item.image_url || "/usato.png"} className="w-full h-full object-cover transition-transform duration-[0.8s]" alt={item.title} />
                    </Link>
            ...
            {!loading && regularItems.length > visibleCount && (
              <div className="mt-12 flex justify-center w-full">
                <button 
                  ...
                  className="bg-stone-900 text-white px-10 py-4 rounded-full text-[11px] font-black uppercase tracking-widest hover:bg-rose-600 transition-all shadow-xl"
                >
        ...
        {/* SIDEBAR BANNER DESTRA (PARTNER) - ALLARGATA A 460px SENZA SFUMATURE */}
        <aside className="hidden lg:flex flex-col gap-6 w-[460px] shrink-0 self-start sticky top-24">
          <div className="bg-white border border-stone-200 rounded-[2rem] p-5 shadow-lg flex flex-col items-center text-center justify-between min-h-[580px] w-full">
            <div className="w-full h-full flex flex-col">
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-[2rem] p-5 shadow-md text-center">
